      ...
      <DialogContent className="max-w-3xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <span>Прогон #{run.runId?.substring(0, 8)}</span>
            <span className={`text-sm font-bold ${statusColor[run.status]}`}>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold">{run.sourceMode}</p>
                </div>
                <div>
                  <span className="text-muted-foreground">Прогресс:</span>
                  <p className="font-semibold">{run.progress || 0}%</p>
                </div>
                <div>
                  <span className="text-muted-foreground">Начало:</span>
                  <p className="font-mono text-xs">{new Date(run.startedAt).toLocaleString()}</p>
                </div>
                {run.finishedAt && (
                  <div>
                    <span className="text-muted-foreground">Окончание:</span>
                    <p className="font-mono text-xs">{new Date(run.finishedAt).toLocaleString()}</p>
                  </div>
                )}
              </div>
            </Card>
            ...
            <Card className="p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  { label: 'Events', value: run.counters?.eventCount || 0 },
                  { label: 'Raw Frames', value: run.counters?.rawFrameCount || 0 },
                  { label: 'Prod Snapshots', value: run.counters?.productSnapshotCount || 0 },
                  { label: 'Unit Economics', value: run.counters?.unitEconomicsCount || 0 },
                  { label: 'Seller Snapshots', value: run.counters?.sellerSnapshotCount || 0 },
                  { label: 'Errors', value: run.counters?.eventsError || 0 },
                  { label: 'Dead Letters', value: run.counters?.deadLetterCount || 0 },
                  { label: 'Processed', value: run.counters?.eventsProcessed || 0 }
                ].map(({ label, value }) => (
            ...
            {run.errors && run.errors.length > 0 && (
              <Card className="p-4">
                <h3 className="font-bold text-base mb-3 flex items-center gap-2">
                  ...
                  Ошибки прогона ({run.errors.length})
                </h3>
                <div className="space-y-2">
